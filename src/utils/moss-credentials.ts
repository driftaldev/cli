...
export async function fetchMossCredentials(): Promise<MossCredentials> {
  ...
  if (isTokenExpired(tokens)) {
    if (tokens.refreshToken) {
      logger.debug(
        "Token expired, refreshing before fetching Moss credentials..."
      );
      ...
      if (tokens?.refreshToken) {
        ...
        if (refreshResult.success && refreshResult.tokens) {
          ...
          if (!retryResponse.ok) {
            ...
            let errorMessage = `Failed to fetch Moss credentials (${retryResponse.status})`;
            try {
          ...
          const credentials = (await retryResponse.json()) as MossCredentials;
